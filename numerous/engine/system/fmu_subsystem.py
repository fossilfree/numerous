...
class FMU_Subsystem1(Subsystem, EquationBase):
    """
    """
    ...
    def __init__(self, fmu_filename: str, tag: str, h: float):
        ...
        def hitground_event_fun1(event_indicators, t, y1, y2):
        ...
        event_ind_call_1_1 = cfunc(types.void(types.voidptr, types.float64, types.float64, types.float64))(
            hitground_event_fun1)
        ...
        def event_cond1(t, y):
            ...
            event_ind_call_1_1(temp_addr, t, y[0], y[1])
            result = carray(temp_addr, (1,), dtype=np.float64)[0]
            return result

        def event_cond_2_1(t, variables):
            q = np.array([variables['t1.h'], variables['t1.v']])
            return event_cond1(t, q)
        ...
        def hitground_event_callback_fun_1(q, a_e):
        ...
        event_ind_call = cfunc(types.void(types.voidptr, types.voidptr))(hitground_event_callback_fun_1)
        ...
        def event_action_2_1(t, variables):
            q = np.array([variables['t1.h'], variables['t1.v']])
            print("------")
            print(t)
            velocity = event_action_1(t, q)
            print(variables['t1.h'])
            print(velocity)
            variables['t1.v'] = velocity

        self.add_event("hitground_event", event_cond_2_1, event_action_2_1, compiled_functions={"event_cond1": event_cond1,
                                                                                            "event_action_1": event_action_1})
...
class S3(Subsystem):
    def __init__(self, tag):
        ...
        fmu_subsystem = FMU_Subsystem1(fmu_filename, "BouncingBall", h=1.6)
