...
from .variables import Variable_old as Variable, VariableType, MappedValue, MappedValue_old
...
class ScopeVariable_old(MappedValue_old):
    """
               Variable inside the scope.
    """
...
class TemporaryScopeWrapper_old:
    ...
    def update_model_from_scope(self, model):
        for scope in self.scope_dict.values():
            for scope_var in scope.variables.values():
                if model.variables[scope_var.id].get_value() != scope_var.value:
